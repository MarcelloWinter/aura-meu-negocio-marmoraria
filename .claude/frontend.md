...
- Helper `getApiErrorMessage(error, fallback)` centraliza a leitura de `error.response.data.message`, usado pelas páginas de Auth, desde 2026-08-27 por `ClientesContext`/`NovoClienteModal`, e desde 2026-08-29 por `Agenda.tsx`/`NovoAgendamentoModal`.
...
- **Agenda.tsx** (`/agenda`): **desde 2026-08-29, não é mais mockado** — busca `GET /agendamentos` (mapeando para `Evento[]`) e `GET /usuarios` (profissionais reais, substituindo a lista fixa "Júlia/Rafa/Você") num `useEffect` cada. Criar (`POST /agendamentos`) e cancelar (`DELETE /agendamentos/:id`) um agendamento também persistem. Grid semanal (8 colunas: 1 de horas + 7 dias) construído com `date-fns` (`startOfWeek`, `addDays`, `addWeeks`, `subWeeks`); eventos posicionados com `position: absolute` calculado a partir de `top`/`height` em função de `HOUR_HEIGHT = 64`. Bloqueios de horário (`Bloqueio[]`, `FechaHorarioModal`) continuam em estado local, sem endpoint. Ver [backend.md](./backend.md) e [business-rules.md](./business-rules.md).
...
- **Este arquivo não recebeu uma reauditoria completa desde 2026-06-25** — foi corrigido pontualmente em 2026-07-30 (roteamento, contextos e a nova página Vendas), em 2026-08-23 (rota/página Configurações, contextos com `clienteId`/dados pessoais de cliente, logout), em 2026-08-27 (Clientes/Atendimento com dados reais, porta do backend, cursor global) e em 2026-08-29 (Agenda com dados reais). As seções sobre build/tooling e páginas de Auth abaixo ainda não foram reconferidas contra o código atual; `business-rules.md` e `changelog.md` são as fontes mais atualizadas.
