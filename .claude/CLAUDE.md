...
3. **A maioria dos módulos de negócio ainda usa dados 100% mockados** (Financeiro, Vendas, Equipe, Configurações), sem integração com o backend. Desde 2026-07-30, Financeiro e Vendas compartilham estado via `FinanceiroContext` (aprovar uma venda gera uma receita no Financeiro), mas isso é só client-side (React Context) — não há persistência real. **Exceção desde 2026-08-27**: Clientes e Atendimento já leem/escrevem dados reais no Postgres (via `GET/POST/DELETE /clientes` e `GET /chats`). **Exceção desde 2026-08-29**: Agenda também (via `GET/POST/DELETE /agendamentos` e `GET /usuarios` para profissionais). Ver ponto 5 abaixo e [business-rules.md](./business-rules.md).
...
5. **O Postgres deste projeto (`DB_NAME=n8n`, `easypanel.aura-ia.cloud`) é compartilhado com uma instância n8n real em produção** — confirmado em 2026-08-27. Por isso, **o backend só pode ler/escrever em tabelas `_copy` (`clientes_copy`, `chats_copy`, `empresas_copy`, `agendamentos_copy`) — nunca nas tabelas live** (regra também salva como memória de projeto, fora do repositório). `usuarios` é exceção: lida diretamente (só `SELECT id, nome`, nunca escrita), pois já era usada por `auth.service.ts`. Ver [database.md](./database.md).
...
- **Agenda passou a usar dados reais**, deixando de ser mockada: novo módulo de backend `agendamentos` (`GET`/`POST`/`DELETE /agendamentos`, sobre `agendamentos_copy`) e `usuarios` (`GET /usuarios`, somente leitura, `id`+`nome`, nunca a senha). `Agenda.tsx` busca e persiste via API; o campo "Profissional" do formulário (antes uma lista fixa de 3 nomes inventados) agora lista usuários reais e ativos.
- **`agendamentos_copy` já existia no banco** antes de qualquer trabalho desta sessão — foi criada por fora, já seguindo a convenção `_copy`. Ganhou duas colunas novas (`observacoes`, `recorrencia`) para acomodar campos que o formulário do frontend já tinha e a tabela não.
- **A instabilidade nas tabelas `_copy` (ver ponto abaixo) se estendeu**: durante os testes de hoje, `agendamentos_copy` também apareceu vazia na maior parte do tempo, e `clientes_copy` continuou variando entre 0 e poucas linhas.
- Detalhes técnicos completos em [backend.md](./backend.md), [database.md](./database.md), [business-rules.md](./business-rules.md), [frontend.md](./frontend.md) e [changelog.md](./changelog.md).

## Pendências gerais

- Confirmar se o nome do negócio de exemplo ("Marmoraria Decore Granitos") é um cliente real ou apenas dado de mock.
- Entender a causa da instabilidade em `chats_copy`/`empresas_copy`/`clientes_copy` (desde 2026-08-27) e agora também `agendamentos_copy` (desde 2026-08-29) antes de recriar os dados ou continuar construindo sobre essas tabelas.
